...
(function () {
  ...
  function postureClass(t) {
    /* DO NOT trust t.posture_class. That field was computed during the
       original base/enrichment split and then FROZEN into dd_v2_base.js by
       the bake, using the earlier classifier that misread #26 Thirukoshtiyur
       and #81 Nagapattinam as Multiple. Reading it here short-circuited
       every correction below -- the console still reported Multiple 8 after
       the fix shipped, because the function returned before running.

       Recompute from the raw posture string and cache under a private key,
       then write the corrected value back so anything else reading
       posture_class sees the right answer too. */
    if (t._ddPosture) return t._ddPosture;
    var p = t.posture;
    if (!p) return (t._ddPosture = null);
    ...
    if (namesEquals) return (t._ddPosture = t.posture_class = 'Multiple');

    /* Otherwise the LEADING posture is the moolavar's posture. */
    if (/^\s*(reclining|kidantha|sayana|bhujanga|shayana)/.test(s)) return (t._ddPosture = t.posture_class = 'Kidantha');
    if (/^\s*(standing|nindra|trivikrama)/.test(s))                 return (t._ddPosture = t.posture_class = 'Nindra');
    if (/^\s*(seated|sitting|veetrirundha|yoga posture|padmasana)/.test(s)) return (t._ddPosture = t.posture_class = 'Veetrirundha');
    ...
    if (multi) return (t._ddPosture = t.posture_class = 'Multiple');

    if (/reclining|sayana|kidantha|bhujanga|shayana|darbhasayana|sthala-sayana/.test(s)) return (t._ddPosture = t.posture_class = 'Kidantha');
    if (/seated|sitting|veetrirundha|yoga posture|padmasana|asana/.test(s)) return (t._ddPosture = t.posture_class = 'Veetrirundha');
    if (/standing|nindra|trivikrama/.test(s)) return (t._ddPosture = t.posture_class = 'Nindra');
    return (t._ddPosture = null);
  }
