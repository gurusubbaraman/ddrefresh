...
(function () {
  ...
  function reconcileCategories() {
    ...
    console.log('[dd_v2_canon] Reconciled ' + stamped + ' stamps across ' +
                Object.keys(CANON).length + ' canonical sets.');
    return stamped;
  }
  ...
  function memberCount(slug) {
    var T = targetArray(); if (!T) return 0;
  ...
  window.DDCanon = {
    ...
    audit: function () {
      var T = targetArray(); if (!T) return null;
  ...
  (function run() {
    if (targetArray()) { reconcileCategories(); }
