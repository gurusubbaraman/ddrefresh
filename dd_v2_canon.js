...
(function () {
  ...
  /* ---------------------------------------------------------------- */
  /* 5b. SYNC v1's window.CATEGORIES  (v1.2)                           */
  /* ---------------------------------------------------------------- */
  /* Reconciling temple.categories[] is necessary but NOT sufficient.
     v1's buildRelatedTab() derives its card from the CATEGORIES table,
     not from the stamps:

         const catData = CATEGORIES[catKey];
         if (catData) {                                  // <- silent drop
           title: `All ${catData.members.length} ${catData.label} temples`

     So the visible count comes from CATEGORIES[slug].members.length --
     the stale, obsolete-numbering array. Measured against CANON:

         divya_kanchi        14 vs 14   correct by coincidence
         thirunangur_cluster 11 vs 11   correct by coincidence
         nava_tirupathi      10 vs  9   WRONG
         pancha_rangam        5 vs  4   WRONG
         pancha_pandava        undefined -> card dropped
         ulagalantha_complex   undefined -> card dropped
         alwar_birthplace      undefined -> card dropped
         sapta_mokshapuri      undefined -> card dropped

     This rewrites the table from CANON so counts are right and all
     eight sets render.

     MUTATE IN PLACE. Section B captured the reference at parse time
     (`const CATEGORIES = window.CATEGORIES;`), so replacing the object
     would orphan it -- the same class of bug the loader v2.2 fixes. */
  var CAT_DESCRIPTIONS = {
    ...
    nava_tirupathi:      'The nine Thamirabarani shrines — also the Navagraha set',
  ...
  function syncLegacyCategories() {
    ...
    /* Drop unverified sets so their cards disappear rather than showing
       a number nobody has stood behind. The per-temple stamps are left
       intact as descriptive tags; only the renderable entry goes. */
    var dropped = [];
    Object.keys(RULINGS_PENDING).forEach(function (slug) {
      if (C[slug]) { delete C[slug]; dropped.push(slug); }
    });

    console.log('[dd_v2_canon] window.CATEGORIES synced: ' + written +
                ' canonical sets rewritten' +
                (dropped.length ? ', ' + dropped.length + ' unverified dropped (' +
                 dropped.join(', ') + ')' : '') + '.');
    return written;
  }
